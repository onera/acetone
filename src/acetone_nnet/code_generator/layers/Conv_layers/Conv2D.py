...
class Conv2D(Layer):
    ...
    def __init__(self, idx: int, conv_algorithm: str, size: int, padding: str | np.ndarray, strides: int, kernel_h: int,
                 kernel_w: int, dilation_rate: int, nb_filters: int, input_shape: list, output_shape: list,
                 weights: np.ndarray, biases: np.ndarray, activation_function: ActivationFunctions):
        super().__init__()
        ...
        self.pad_right, self.pad_left, self.pad_bottom, self.pad_top = self.compute_padding(self.padding,
                                                                                            self.input_height,
                                                                                            self.input_width,
                                                                                            self.kernel_h,
                                                                                            self.kernel_w, self.strides,
                                                                                            self.dilation_rate)
        ...
        if not isinstance(self.activation_function, ActivationFunctions):
            raise TypeError(
                "Error: activation function type in Conv2D (activation function must be a sub-classe of acetone_nnet Activation Function)")

        ### Checking value consistency ###
        if self.size != self.output_channels * self.output_height * self.output_width:
            raise ValueError("Error: size value in Conv2D (" + str(self.size) + "!=" + str(
        ...
        if self.weights.shape != (self.input_channels, self.kernel_h, self.kernel_w, self.nb_filters):
            raise ValueError("Error: non consistency between weight shape and operation parameters in Conv2D (" + str(
                self.weights.shape) + "!=" + str(
                (self.input_channels, self.kernel_h, self.kernel_w, self.nb_filters)) + ")")
        if len(self.biases.shape) != 1 or self.biases.shape[0] != self.nb_filters:
            raise ValueError("Error: non consistency between the var shape and the output shape in Conv2D (" + str(
                self.biases.shape) + "!=" + str(self.nb_filters) + ")")
        if self.output_height != math.floor((self.input_height + self.pad_bottom + self.pad_top - self.kernel_h - (
            ...
            raise ValueError(
                "Error: non consistency between the output height and the parameter of the operation in Conv2D (" + str(
                    self.output_height) + "!=" + str(math.floor((
                                                                        self.input_height + self.pad_bottom + self.pad_top - self.kernel_h - (
                                                                        self.kernel_h - 1) * (
                                                                                self.dilation_rate - 1)) / self.strides) + 1) + ")")
        if self.output_width != math.floor((self.input_width + self.pad_left + self.pad_right - self.kernel_w - (
            ...
            raise ValueError(
                "Error: non consistency between the output width and the parameter of the operation in Conv2D (" + str(
                    self.output_width) + "!=" + str(math.floor((
                                                                       self.input_width + self.pad_left + self.pad_right - self.kernel_w - (
                                                                       self.kernel_w - 1) * (
                                                                               self.dilation_rate - 1)) / self.strides) + 1) + ")")
    ...
    def forward_path_layer(self, input: np.ndarray):
        ...
        if self.pad_right or self.pad_left or self.pad_top or self.pad_bottom:
            ...
            input_padded[:, self.pad_top:border_bottom, self.pad_left:border_right] = input
        else:
            input_padded = input
        ...
        for f in range(self.nb_filters):
            for i in range(self.output_height):
                for j in range(self.output_width):
                    output[f, i, j] = np.sum(
                        input_padded[:, i * self.strides:i * self.strides + self.kernel_h,
                        j * self.strides:j * self.strides + self.kernel_w + self.pad_left]
