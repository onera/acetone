...
class CodeGenerator(ABC):
    ...
    def __init__(self,
        ...
        if (not self.normalize):
            ...
            l, dtype, dtype_py, data_format, maxpath, dict_cst, Normalizer = parser(self.file, self.conv_algorithm,
        ...
        self.files_to_gen = ["inference.c", "inference.h", "global_vars.c", "main.c", "Makefile", "test_dataset.h",
                             "test_dataset.c"]
        ...
        if not isinstance(self.file, str | Path | onnx.ModelProto | Functional | Sequential):
    ...
    def create_test_dataset(self):
        test_dataset = self.data_type_py(
            np.random.default_rng(seed=10).random((self.nb_tests, 1, int(self.layers[0].size))))
        return test_dataset
        ...
        with open(os.path.join(c_files_directory, "output_python.txt"), "w+") as fi:
            for nn_input in self.test_dataset:
                ...
                if ((self.data_format == "channels_last") and (len(self.layers[0].input_shape) == 4)):
                    shape = (
                    self.layers[0].input_shape[2], self.layers[0].input_shape[3], self.layers[0].input_shape[1])
                    nn_input = np.transpose(np.reshape(nn_input, shape), (2, 0, 1))

                if (self.normalize): nn_input = self.Normalizer.pre_processing(nn_input)
                previous_layer_result = [nn_input for i in range(
                    self.maxpath)]  # for the very first layer, it is the neural network input

                to_store = {}  #a dictionnary containing the values to store
                for layer in self.layers:
                    if (not layer.previous_layer):
                        previous_layer_result[layer.path] = layer.forward_path_layer(previous_layer_result[
                                                                                         layer.path])  #if the layer is an input layer, it directly take the vaue from it's path
                    elif (len(layer.previous_layer) == 1):
                        if (len(layer.previous_layer[0].next_layer) == 1):
                            previous_layer_result[layer.path] = layer.forward_path_layer(previous_layer_result[
                                                                                             layer.previous_layer[
                                                                                                 0].path])  #if the layer has exactly one previous_layer, it takes the value from it's father's path
                        else:
                            previous_layer_result[layer.path] = layer.forward_path_layer(to_store[layer.previous_layer[
                                0].idx])  #if the father is stored, we take it from the storage
                    ...
                    for prev in layer.previous_layer:
                        if ((prev.sorted == len(prev.next_layer)) and (
                                prev in to_store)):  #if all the children of the parent layer are "taken care of", we "forget" the parent's value ( *2 because of the creation of the dict in graph.to_save)
                        ...
                        if layer.name != "Input_layer":
                            if layer.idx in self.debug_target:
                                ...
                                if ((self.data_format == "channels_last") and hasattr(layer, "output_channels")):
                                    debug_output[-1] = np.transpose(debug_output[-1], (1, 2, 0))
                ...
                if ((self.data_format == "channels_last") and hasattr(layer,
                                                                      "output_channels")): nn_output = np.transpose(
                    nn_output, (1, 2, 0))
    ...
    def generate_testdataset_files(self, output_dir: Path):
        ...
        with open(self.template_path + "template_test_dataset_header.c.tpl") as template_file:
            template = template_file.read()
        template_file.close()

        testdataset_header.write(pystache.render(template, {"nb_tests": self.nb_tests, "nb_inputs": self.layers[0].size,
        ...
        with open(self.template_path + "template_test_dataset_source.c.tpl") as template_file:
            template = template_file.read()
        template_file.close()
    ...
    def generate_makefile(self):
        ...
        self.makefile.write(pystache.render(template, {"source_files": " ".join(source_files),
    ...
    def generate_function_source_file(self, output_dir: Path):
        ...
        if (any(isinstance(layer, Conv2D_6loops) or isinstance(layer, AveragePooling2D) or isinstance(layer, Softmax))
            for layer in self.layers):
        ...
        mustach_hash["layers"] = []
        for layer in self.layers:
            layer_hash = {"inference_function": layer.generate_inference_code_layer(), "path": layer.path,
                          "size": layer.size}
        ...
        if ((self.data_format == "channels_last") and (hasattr(self.layers[-1], "output_channels"))):
            ...
            with open(self.template_path + "memory_layout/template_channels_last_output.c.tpl") as template_file:
                template = template_file.read()
            template_file.close()
            ...
            if (self.data_format == "channels_first"):
                output_hash[
                    "comment"] = "Returning the output in channels first (ACETONE compute the result in channels first)"
            else:
                output_hash["comment"] = "Returning the output (output flatten)"

            with open(self.template_path + "memory_layout/template_channels_first_output.c.tpl") as template_file:
                template = template_file.read()
            template_file.close()
            mustach_hash["ouput_str"] = pystache.render(template, output_hash)
    ...
    def generate_globalvars_file(self):
        ...
        with open(self.template_path + "template_global_var_file.c.tpl") as template_file:
            template = template_file.read()
        template_file.close()
