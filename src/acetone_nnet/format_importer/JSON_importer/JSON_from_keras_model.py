"""Generate a JSON file for ACETONE from a keras model.
...
def json_from_keras_model(
        keras_model: Functional | Sequential,
        output_dir_json: str | Path,
) -> None:
    """Generate a .json file from a keras model."""
    ...
    if (hasattr(layer["config"], "data_format")
            and (layer["config"]["data_format"] == "channels_last")
    ...
    if keras_model.layers[0].__class__.__name__ == "InputLayer":
        ...
        if model_dict["config"]["data_format"] == "channels_last":
            if (type(keras_model.input_shape) is list
                    and len(keras_model.input_shape[0]) == 4):
                model_dict["config"]["layers"][0]["config"]["input_shape"] = [
                    (shape[0], shape[3], shape[1], shape[2])
                    for shape in keras_model.input_shape
                ]
            elif ((type(keras_model.input_shape) is not list)
                  and (len(keras_model.input_shape) == 4)):
                model_dict["config"]["layers"][0]["config"]["input_shape"] = (
                    keras_model.input_shape[0],
                    keras_model.input_shape[3],
                    keras_model.input_shape[1],
                    keras_model.input_shape[2],
                )
            else:
    ...
    i = 0
    idx = start
    for layer_json, layer_keras \
            in zip((model_dict["config"]["layers"])[start:], keras_model.layers, strict=False):
        ...
        if model_dict["config"]["data_format"] == "channels_last":
            ...
            if (type(input_shape) is list) and (len(input_shape[0]) == 4):
                input_shape = [
                    (shape[0], shape[3], shape[1], shape[2]) for shape in input_shape
                ]
        ...
        for j in range(1, len(output_shape)):
            # start at idx 1 cause idx 0 represents batch size, so it's None
        ...
        if layer_json.get("inbound_nodes"):
    ...
    with Path.open(Path(output_dir_json), "w", encoding="utf-8") as f:
