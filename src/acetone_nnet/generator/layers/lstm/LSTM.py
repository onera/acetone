...
class LSTM(Layer):
    ...
    def __init__(
        ...
        if type(self.idx) is not int:
            msg += "Error: idx type in LSTM (idx must be int)"
            msg += "\n"
        if type(self.size) is not int:
            msg += "Error: size type in LSTM (size must be int)"
            msg += "\n"
        if type(self.output_height) is not int:
            msg += "Error: output height type in LSTM (must be int)"
            msg += "\n"
        if type(self.output_width) is not int:
            msg += "Error: output width type in LSTM (must be int)"
            msg += "\n"
        if type(self.input_height) is not int:
            msg += "Error: input height type in LSTM (must be int)"
            msg += "\n"
        if type(self.input_width) is not int:
            msg += "Error: input width type in LSTM (must be int)"
            msg += "\n"
        if type(self.weights) is not np.ndarray:
            msg += "Error: weights in LSTM (weights must be an numpy array)"
            msg += "\n"
        if type(self.biases) is not np.ndarray:
            msg += "Error: biases in LSTM (biases must be an numpy array)"
            msg += "\n"
        if msg:
            raise TypeError(msg)
        ...
        if self.size != self.output_height * self.output_width:
            msg += f"Error: size value in LSTM ({self.size}!={self.output_height * self.output_width})"
            msg += "\n"
        shape = self.input_height if self.transpo[0] else self.input_width
        if self.weights.shape[self.transpo[1]] != shape:
            msg += f"Error: non consistency between weight shape and input shape in LSTM ({self.weights.shape[self.transpo[1]]}!={shape})"
            msg += "\n"
        shape = self.input_width if self.transpo[0] else self.input_height
        if self.output_height != shape:
            msg += f"Error: non consistency between input shape and output shape in LSTM ({self.output_height}!={shape})"
            msg += "\n"
        if self.output_width != self.weights.shape[1 - self.transpo[1]]:
            msg += f"Error: non consistency between output shape and output weight in LSTM ({self.output_width}!={self.weights.shape[1 - self.transpo[1]]})"
            msg += "\n"
        if any(
            self.biases.shape[i] != 1 and self.biases.shape[i] != output_shape[3 - i]
            for i in range(len(self.biases.shape))
        ):
            msg = f"Error: biases ({self.biases.shape}) in LSTM not broadcastable to dim ({self.output_height},{self.output_width})"
            msg += "\n"
        if msg:
    ...
    def forward_path_layer(
        ...
        return self.activation_function.compute(
            np.dot(input_array, w) + self.biases,
        )
