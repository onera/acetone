...
class ReduceMax(Layer):
    ...
    def __init__(
        ...
        for axe in self.axes:
            if axe < 0 or axe >= 4:
                msg += (
                    f"Error: axis out of bound in Reduce "
    ...
    def forward_path_layer(
        ...
        if self.axes == ():
            if self.noop_with_empty_axes:
                return input_array
            return np.max(input_array, axis=None, keepdims=self.keepdims)
        return np.max(input_array, axis=self.axes, keepdims=self.keepdims)
