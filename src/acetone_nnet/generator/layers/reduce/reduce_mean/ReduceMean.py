"""ReduceMean layer type definition.
...
class ReduceMean(Layer):
    ...
    def __init__(
        ...
        """Instantiate a ReduceMean layer."""
        Layer.__init__(self)
        self.idx = idx
        self.size = size
    ...
    def forward_path_layer(
        ...
        if self.axes == ():
            if self.noop_with_empty_axes:
                return input_array
            return np.mean(input_array, axis=None, keepdims=self.keepdims)
        return np.mean(input_array, axis=self.axes, keepdims=self.keepdims)
