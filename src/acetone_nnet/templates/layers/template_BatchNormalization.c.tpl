    ...
    for(f = 0; f < {{input_channels}}; ++f)
    {
        ...
        {
            output_{{path}}[k + {{channel_size}}*f] = scale_{{name}}_{{idx}}[f]*(ctx->{{output_str}}[k + {{channel_size}}*f] - mean_{{name}}_{{idx}}[f])/sqrt(var_{{name}}_{{idx}}[f] + {{epsilon}}) + biases_{{name}}_{{idx}}[f];
            {{#activation_function}}
