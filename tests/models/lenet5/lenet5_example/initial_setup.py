...
model.save('./tests/models/lenet5/lenet5_example/lenet5.h5')
print("Neural network model exported to acetone/tests/models/lenet5/lenet5_example/lenet5.h5")

### Define a random input of shape (1,1,28*28*1) ###
random_input = np.random.default_rng().random((1,28, 28, 1))
with open('./tests/models/lenet5/lenet5_example/test_input_lenet5.txt', 'w') as filehandle:
    for i in range(random_input.shape[0]):
        row = (random_input[i]).flatten(order='C')
        json.dump(row.tolist(), filehandle)
        filehandle.write('\n')
filehandle.close()
print("Sample input data exported to acetone/tests/models/lenet5/lenet5_example/test_input_lenet5.txt")
...
### Export Keras' output ###
with open('./tests/models/lenet5/lenet5_example/output_keras.txt', 'w+') as fi:
...
print("Keras' inference output exported to acetone/tests/models/lenet5/lenet5_example/output_keras.txt")
