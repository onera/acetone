...
class TestAcasFullyConnected(importerTestCase.ImporterTestCase):
    ...
    def test_AcasFullyConnected(self):
        model_path = '/'.join(__file__.split('/')[:-3])+'/models/acas/acas_fully_connected/acas_fully_connected.onnx'
    ...
    def test_AcasFullyConnectedNormalized(self):
        model_path = '/'.join(__file__.split('/')[:-3])+'/models/acas/acas_fully_connected/acas_fully_connected_normalized.onnx'
        model = onnx.load_model(model_path)
