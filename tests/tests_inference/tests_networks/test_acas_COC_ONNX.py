...
class TestAcas_COC_ONNX(acetoneTestCase.AcetoneTestCase):
    ...
    def testAcas_COC_ONNX(self):
        model = onnx.load('./tests/models/acas/acas_COC/nn_acas_COC.onnx')
        ...
        sess = rt.InferenceSession('./tests/models/acas/acas_COC/nn_acas_COC.onnx')
        input_name = sess.get_inputs()[0].name
        result = sess.run(None,{input_name: dataset[0]})
        onnx_result = result[0].ravel().flatten()
        acetone_result = acetoneTestCase.run_acetone_for_test(self.tmpdir_name,'./tests/models/acas/acas_COC/nn_acas_COC.onnx', self.tmpdir_name+'/dataset.txt')
