...
class TestAcas_fully_connected_ONNX(acetoneTestCase.AcetoneTestCase):
    ...
    def testAcas_fully_connected_ONNX(self):
        model = onnx.load(test_path +'/models/acas/acas_fully_connected/acas_fully_connected.onnx')
        ...
        sess = rt.InferenceSession(test_path +'/models/acas/acas_fully_connected/acas_fully_connected.onnx')
        input_name = sess.get_inputs()[0].name
        result = sess.run(None,{input_name: dataset[0]})
        onnx_result = result[0].ravel().flatten()
        acetone_result = acetoneTestCase.run_acetone_for_test(self.tmpdir_name,test_path +'/models/acas/acas_fully_connected/acas_fully_connected.onnx', self.tmpdir_name+'/dataset.txt')

        self.assertListAlmostEqual(list(acetone_result[0]), list(onnx_result))
    
    def testAcas_fully_connected_ONNXPython(self):
        acetone_result = acetoneTestCase.run_acetone_for_test(self.tmpdir_name,test_path +'/models/acas/acas_fully_connected/acas_fully_connected.onnx')
