...
class ConcreteProductSystemACASXU_2(AbstractProductSystem):
    """A class corresponding to the 2D ACASXU_2 system and the associated simulation options, implementing the AbstractProductSystem class
    ...
    def simulate_dynamics_euler(self, plant_state, command) -> np.array:
        ...
        while n_steps_simulated < n_integration_steps:
            ...
            new_psi_int = cur_state[3] + command[1] * delta_t
    ...
    def pre_process(self, id_agent, plant_state) -> np.array:
        ...
        if delta_y > 0:
            ...
            angle = 0 if y==0 else pi
        if id_agent==0:
            theta = angle - psi_own
        elif id_agent==1:
            theta = angle - psi_int + pi
        ...
        if id_agent==0:
            ...
            return np.array([rho, theta, psi, v_int, v_own])
...
class ConcreteFactoryACASXU_2(AbstractFactorySystem):
    def createSystem(self, config_system) -> ConcreteProductSystemACASXU_2:
        return ConcreteProductSystemACASXU_2(config_system)
